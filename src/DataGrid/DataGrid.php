...
class DataGrid extends \Nette\Application\UI\Control implements \ArrayAccess
{
	...
	/** @var array */
	public $displayedItems = array(5, 10, 15, 20, 50, 100);
	...
	public function setDataSource(DataSources\IDataSource $dataSource)
	{
		...
		if($dataSource->getKeyName() !== null)
			$this->keyName = $dataSource->getKeyName();
		return $this;
	}
	...
	protected function finalize()
	{
		...
		if ($this->presenter->isAjax()) {
			...
			$snippet = 'snippet-' . $this->getUniqueId() . '-';
...
//		Nette\Forms\Controls\BaseControl::$idMask = 'frm-datagrid-' . $this->getUniqueId() . '-%s-%s';
		...
		$ipp = $form->addSelect('items', 'Items per page', array_combine($this->displayedItems, $this->displayedItems));
		$ipp->setTranslator(null);
	 ...
	 * 
	 * @param string $title
	 * @param funcion|callback $linkCallback	function($data)
	 * @param Html $icon
	 * @param bool $useAjax
	 * @return CallbackAction
	 * @throws \InvalidStateException
	 */
	public function addCallbackAction($title, $linkCallback, $icon = NULL, $useAjax = FALSE)
	{
		if (!$this->hasColumns('DataGrid\Columns\ActionColumn')) {
			throw new \InvalidStateException('No DataGrid\Columns\ActionColumn defined. Use DataGrid\DataGrid::addActionColumn before you add actions.');
		}

		return $this->currentActionColumn->addCallbackAction($title, $linkCallback, $icon, $useAjax);
	}
